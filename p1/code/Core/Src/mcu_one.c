...
SemaphoreHandle_t mutex_handle_receive_from_MCU_two_on_MCU_one;
...
void init_MCU_one() {
	...
	xTaskCreate(task_function_receive_from_MCU_two, "receive_from_MCU2", 64, NULL, 20, &task_handle_receive_from_MCU_two_on_MCU_one);
	queue_handle_receive_from_MCU_two_on_MCU_one = xQueueCreate(32, sizeof(uint8_t));
	mutex_handle_receive_from_MCU_two_on_MCU_one = xSemaphoreCreateMutex();
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
	...
	if(huart->Instance == huart2.Instance) {
		vTaskNotifyGiveFromISR(task_handle_receive_from_MCU_two_on_MCU_one, &woken);
	}
...
void task_function_send_to_MCU_two(void* params) {
	...
	while(1) {
		...
		HAL_UART_Transmit_IT(&huart2, &item_to_send_to_MCU_two, sizeof(uint8_t));
		ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
	}
}
...
void task_function_receive_from_MCU_two(void* params) {
	MotorCommand item_received_from_MCU_two;
	while(1) {
		HAL_UART_Receive_IT(&huart2, &item_received_from_MCU_two.motor, sizeof(uint8_t));
		ulTaskNotifyTake(pdTRUE, portMAX_DELAY);

		HAL_UART_Receive_IT(&huart2, &item_received_from_MCU_two.velocity, sizeof(uint8_t));
		ulTaskNotifyTake(pdTRUE, portMAX_DELAY);

		modify_velocities(item_received_from_MCU_two);
	}
}

void modify_velocities(MotorCommand m) {
	xSemaphoreTake(mutex_handle_receive_from_MCU_two_on_MCU_one, portMAX_DELAY);

	velocities[m.motor] = m.velocity;

	xSemaphoreGive(mutex_handle_receive_from_MCU_two_on_MCU_one);
}
